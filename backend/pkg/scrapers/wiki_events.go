...
// EnhanceRecentEventsWithWikiData enhances only events from the past 2 months and future events
func (s *WikiEventScraper) EnhanceRecentEventsWithWikiData(ctx context.Context, cutoffDate time.Time) (int, error) {
    query := `
        SELECT id, name, event_date, venue, city, country, ufc_url, status, wiki_url, attendance
        FROM events
        WHERE event_date >= $1 OR event_date >= CURRENT_DATE
        ORDER BY event_date
    `
    ...
    if err := rows.Err(); err != nil {
        return 0, fmt.Errorf("error iterating event rows: %w", err)
    }

    log.Printf("Loaded %d recent events from database", len(s.existingEvents))
...
// loadRecentEvents loads only recent events (after cutoffDate) from the database
