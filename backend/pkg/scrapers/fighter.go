...
func (s *FighterScraper) ScrapeFighters() ([]*models.Fighter, error) {
    ...
    // Get initial page and view_dom_id
    ...
    // Results collector with progress tracking
    totalProcessed := 0
    go func() {
        for fighter := range results {
            ...
            if totalProcessed%50 == 0 {
                fmt.Printf("Processed %d fighters so far\n", totalProcessed)
            }
            mutex.Unlock()
        }
    }()
    ...
    for page < maxPages {
        ...
        fmt.Printf("Found %d fighters on page %d\n", len(pageURLs), page)

        // Queue up fighter URLs for processing
        for _, url := range pageURLs {
            jobs <- url
        }

        if len(pageURLs) == 0 {
            // If we didn't find any fighter URLs, probably reached the end
...
// ScrapeFighter gets detailed info for a single fighter
func (s *FighterScraper) ScrapeFighter(url string) (*models.Fighter, error) {
    ...
    // Get nickname (remove quotes)
    fighter.Nickname = strings.TrimSpace(doc.Find("p.hero-profile__nickname").Text())
    fighter.Nickname = strings.Trim(fighter.Nickname, "\"")

    // Get division and record
    fighter.WeightClass = strings.TrimSpace(strings.Replace(
        doc.Find(".hero-profile__division-title").Text(),
        " Division",
        "",
